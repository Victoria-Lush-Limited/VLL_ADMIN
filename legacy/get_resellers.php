...
$q = mysqli_query($conn, "SELECT * FROM resellers ORDER BY date_created DESC");
...
<table>
    ...
    <?php
    $q = mysqli_query($conn, "SELECT * FROM resellers ORDER BY date_created ASC LIMIT " . $start_row . "," . $per_page);
    if ($found) {
        while ($reseller = mysqli_fetch_assoc($q)) {
            $table_rows += 1;
            
            $b = mysqli_query($conn, "SELECT (SUM(allocated)-SUM(consumed)) AS balance FROM transactions WHERE user_id='" . $reseller['user_id'] . "'");
    ...
    ?>
            <tr style="background-color:  <?php echo $bg; ?>;  font-size:var(--small-text);">
                ...
                <td><?php echo $reseller['user_id']; ?></td>
                <td align="right"><?php echo number_format($balance); ?></td>
                <td><?php echo $reseller['status']; ?></td>
                <td>
                    <div class="row-options">
                        <a href="reseller_account.php?reseller_id=<?php echo $reseller['user_id']; ?>"><i class="fas fa-user-cog fa-lg" style="color:var(--blue-color);"></i></a>
                    </div>
                </td>
            </tr>
    <?php
